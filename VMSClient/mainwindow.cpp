...
{
    QFile dataFile("./_viewsetting.db");
...
void MainWindow::setupCameraViews()
{
    ...
    for(int i = 0; i < CAMERA_VIEW_MAX_ROW; i ++)
    {
        ...
        {
            CameraSurfaceGLView* surfaceView = new CameraSurfaceGLView();
            hLayout->addWidget(surfaceView);
            hLayout->setMargin(0);
            hLayout->setSpacing(0);

            connect(surfaceView, SIGNAL(maximumChanged()), this, SLOT(slotMediaMaximumChanged()));
            connect(surfaceView, SIGNAL(selected(CameraSurfaceGLView*)), this, SLOT(slotMediaSelectionChanged(CameraSurfaceGLView*)));
...
void MainWindow::slotCapture()
{
    ...
    for(int i = 0; i < m_cameraViewItems.size(); i ++)
    {
        if(m_cameraViewItems[i]->isSelected() && m_cameraViewItems[i]->isRunning())
        {
            exist = i;
            break;
        }
...
{
    CameraSurfaceGLView* selectedMediaItem = NULL;
...
void MainWindow::slotMediaSelectionChanged(CameraSurfaceGLView* selectedView)
{
    for(int i = 0; i < m_cameraViewItems.size(); i ++)
...
void MainWindow::retranslateUI()
{
    ...
    setWindowTitle(StringTable::Str_VMSClient);
}
...
void MainWindow::slotExportBlack()
{
    ...
    QSettings setting(QSettings::IniFormat, QSettings::SystemScope, ORG_NAME, APP_NAME);
    QString selectedFileName = QFileDialog::getSaveFileName(this, StringTable::Str_Save_File, setting.value("export black dir").toString() + "black info.db", "Black Info(*.db)");
