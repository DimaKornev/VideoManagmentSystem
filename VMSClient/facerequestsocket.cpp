...
{
    char receiveDataPacket[1024] = { 0 };
    while(1)
    {        
        int ret = recv(m_socket, receiveDataPacket, sizeof(receiveDataPacket), 0);
        ...
        QByteArray receiveData(receiveDataPacket, ret);
        ...
        emit receivedFaceResult(faceResults, imageTime, serverTime - QDateTime::currentDateTime().toMSecsSinceEpoch());
    }

    QVector<QRect> emptyResults;
    emit receivedFaceResult(emptyResults, 0, 0);
}
