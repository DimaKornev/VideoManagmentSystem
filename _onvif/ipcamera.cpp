...
void cameraFrameProc(void* param);
    ...
    QObject(parent)
{
    ...
    connect(this, SIGNAL(requestReconnect()), this, SLOT(reconnect()));
}
...
{
    m_connecting = 1;
    m_frameThread = (HANDLE)_beginthread(cameraFrameProc, 0, this);
}
...
void IpCamera::frameProc()
{
    ...
    if(m_connecting)
    {
        ...
        while(isRunning())
        {
            ...
            if(oldTime.msecsTo(QTime::currentTime()) > 20)
            {
                m_frameMutex.lock();
...
YUVFrame IpCamera::readFrame()
{
    ...
    YUVFrame yuvFrame;
    yuvFrame.yuvData = m_frameData[m_index];
    yuvFrame.width = m_dec_ctx->width;
    yuvFrame.height = m_dec_ctx->height;
    yuvFrame.frameTime = frameTime;
...
void IpCamera::setReceiveFrameProc(int sendAddr, fnReceiveFrameProc* proc)
{
    m_sender = sendAddr;
    m_receiveFrameProc = proc;
}

void cameraFrameProc(void* param)
{
    IpCamera* cameraSurface = (IpCamera*)param;
    cameraSurface->frameProc();
    _endthread();
}
