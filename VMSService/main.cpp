...
#define BUF_SIZE 256
TCHAR szName[]=L"Global\\VMSServiceMap";
...
VOID CALLBACK ForceExitCallback(PVOID, BOOLEAN)
{
    ...
    // Ack the request
    hForceExitAck = OpenEvent(EVENT_ALL_ACCESS, FALSE, L"Global\\EasenVMSForceExitAck");
    if (hForceExitAck)
    {
...
int ProviderMain(QApplication* app)
{
    ...
    if (ok && (hProviderStart = OpenEvent(EVENT_ALL_ACCESS, FALSE, L"Global\\EasenVMSProviderStart")))
    {
    ...
    if (ok && (hForceExit = OpenEvent(EVENT_ALL_ACCESS, FALSE, L"Global\\EasenVMSForceExit")))
    {
...
VOID WINAPI ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv)
{
    ...
    if (ok)
    {
        ...
        ForceExit = CreateEvent(NULL, FALSE, FALSE, L"Global\\EasenVMSForceExit");
        ForceExitAck = CreateEvent(NULL, FALSE, FALSE, L"Global\\EasenVMSForceExitAck");
        ProviderStart = CreateEvent(NULL, FALSE, FALSE, L"Global\\EasenVMSProviderStart");
    }
