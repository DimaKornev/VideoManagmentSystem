...
void CameraProcessEngine::processFaceDetection()
{
    ...
    while(m_faceDetectionRunning)
    {
        ...
        sendFaceRects(faceRects, imageTime);
...
void CameraProcessEngine::processModelDetection()
{
    ...
    while(m_modelDetectionRunning)
    {
        ...
        if(faceCount)
        {
            ...
            //ConvertYUV2RGB((unsigned char*)yuvData.data(), width, height, srcImage.bits(), m_chanelIndex);
...
void CameraProcessEngine::sendFaceRects(QVector<QRect> faceRects, qint64 frameTime)
{
    QMutexLocker locker(&m_faceRectSendingMutex);
